-- Events foundation (F11, DM §11.5).
--
-- Household calendar commitments (vet appointments, classes, grooming, other)
-- that feed Planner and plan-generation context. Clients SELECT only; mutations
-- go through write_path_* SECURITY DEFINER RPCs.
--
-- This migration ALSO replaces write_path_generation_context so `events` is no
-- longer hard-coded to []. It starts from the composed
-- 20260728001200_restore_training_state_in_context body and ONLY changes the
-- events array — training_state, socialization history, and catalogue fields
-- are preserved (docs/22 handoff trap).
